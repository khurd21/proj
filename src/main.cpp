...
compdef _proj proj
    ...
    proj::ConfigSettings settings{"config/proj.yaml"};

    if (cli_args.list_projects) {
      const auto projects{proj::list_projects(settings.scan_directories)};
    ...
    const auto project{proj::find_project(settings.scan_directories, cli_args.project_name)};
    if (project.has_value()) {
      return proj::open_in_editor(settings.editor, project.value());
    }
